...
PHP 8.5 là phiên bản mặc định. `docker compose up -d` chỉ khởi động PHP 8.5; PHP 7.4, 8.0, 8.1, 8.2, 8.3 và 8.4 được đặt trong profile riêng và mặc định không chạy. MySQL, PostgreSQL, Redis, RabbitMQ, Kafka và Supervisor cũng nằm trong profile riêng: mặc định không khởi động cho đến khi bạn bật profile tương ứng.
...
Lệnh này khởi động PHP 8.5, Nginx, Server Manager và PHP Controller. PHP tùy chọn (7.4, 8.0, 8.1, 8.2, 8.3, 8.4), MySQL, PostgreSQL, Redis, RabbitMQ, Kafka và Supervisor mặc định tắt — bật chúng từ Server Manager.
...
Dùng UI này để quản lý virtual server, phiên bản PHP, hosts, Nginx, MySQL, PostgreSQL, Redis, RabbitMQ, Kafka và Supervisor. Thứ tự lần đầu:
...
- Khởi động và dừng **MySQL**, **PostgreSQL**, **Redis**, **RabbitMQ**, **Kafka** và **Supervisor** từ UI.
...
### Profile PHP, MySQL, PostgreSQL, Redis, RabbitMQ, Kafka và Supervisor
...
MySQL, PostgreSQL, Redis, RabbitMQ và Kafka:
...
Tên service hợp lệ: `env-init`, `nginx`, `php-8.5`, `php-8.4`, `php-8.3`, `php-8.2`, `php-8.1`, `php-8.0`, `php-7.4`, `supervisor-8.5`, `supervisor-8.4`, `supervisor-8.3`, `supervisor-8.2`, `supervisor-8.1`, `supervisor-8.0`, `supervisor-7.4`, `manager`, `php-controller`, `mysql`, `postgres`, `redis`, `rabbitmq`, `kafka`.
...
Supervisor dùng hostname `mysql`, `postgres`, `redis`, `rabbitmq` và `kafka` để kết nối các dịch vụ trong `app-network`. `depends_on` với `required: false` chỉ sắp thứ tự khởi động khi các profile MySQL/PostgreSQL/Redis/RabbitMQ/Kafka đang bật; không đảm bảo dịch vụ đã sẵn sàng nhận kết nối — worker nên có cơ chế retry.

### Dùng Supervisor với phiên bản PHP khác

Mỗi container Supervisor chỉ có một PHP runtime. PHP (+ Supervisor) nằm trong `compose/php-X.Y.yml`. MySQL, PostgreSQL, Redis, RabbitMQ và Kafka dùng chung, khai báo ở `compose/mysql.yml`, `compose/postgres.yml`, `compose/redis.yml`, `compose/rabbitmq.yml` và `compose/kafka.yml`. Root `docker-compose.yml` `include` các file đó.
