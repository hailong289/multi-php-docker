...
# Sync existing /etc/nginx/templates → conf.d and reload Nginx.
# Does NOT regenerate templates from env.json (preserves manual edits).
...
: > "$LOG_FILE"
echo "Soft-reload: syncing templates → conf.d (no env regenerate)" >> "$LOG_FILE"

find /etc/nginx/conf.d -maxdepth 1 -type f -name '*.conf' -delete
for template in /etc/nginx/templates/*.template; do
    [ -e "$template" ] || break
    cp "$template" "/etc/nginx/conf.d/$(basename "${template%.template}.conf")"
    echo "Synced: $template" >> "$LOG_FILE"
done
...
write_status "success" "Nginx templates were synced and reloaded successfully."
