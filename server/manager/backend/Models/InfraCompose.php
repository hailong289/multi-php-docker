 ...
 * Core service files (mysql|postgres|redis|rabbitmq|kafka|mailpit|minio).yml cannot be deleted.
 */
final class InfraCompose
{
    ...
    /** @var list<string> */
    public const CORE_FILES = ['mysql.yml', 'postgres.yml', 'redis.yml', 'rabbitmq.yml', 'kafka.yml', 'mailpit.yml', 'minio.yml'];
    ...
    public function actionContextForFile(string $name, ?string $content = null): ?array
    {
        ...
        if (preg_match('/^(mysql|postgres|redis|rabbitmq|kafka|mailpit|minio)$/i', $service)) {
    ...
    {
        return <<<'YAML'
# Custom Compose fragment — rename "example" to match your service (e.g. meilisearch).
